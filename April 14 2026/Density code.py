...
# ║  SECTION 32 — FIGURE 9: HALF-MASS RADIUS AND STRUCTURAL SCALARS           ║
...
ax_rho33.set_xlabel(“r [kpc]”); ax_rho33.set_ylabel(r”$\rho$ [M$*\odot$ kpc$^{-3}$]”)
ax_rho33.set_title(r”$\rho(r)$  history”)
ax_gal33.set_xlabel(“r [kpc]”); ax_gal33.set_title(r”$\rho*{\rm MW}$ vs. $\rho_{\rm M31}$”)
...
ax00.set_ylabel(r”$\rho$ [M$*\odot$/kpc³]”, fontsize=8, color=MUTED)
ax00.set_title(r”$\rho(r)$  at 5 epochs”, fontsize=9)
ax00.legend(fontsize=6)

# ── Panel (0,1): Σ(R) ─────────────────────────────────────────────────────────

ax01 = *sax(fig35, gs35, 0, 1)
for k_idx, color, label in zip(PROFILE_INDICES, PROFILE_COLORS, PROFILE_LABELS):
y = Sigma_ts[k_idx, :]
v = np.isfinite(y) & (y > 0)
if v.any():
...
ax01.set_ylabel(r”$\Sigma$ [M$*\odot$/kpc²]”, fontsize=8, color=MUTED)
...
ax11 = *sax(fig35, gs35, 1, 1, log_x=False, log_y=False)
...
ax11.set_title(r”$f*{\rm mix}(r,t)$ mixing fraction”, fontsize=9)
fig35.colorbar(im11, ax=ax11, label=r”$f_{\rm mix}$”, shrink=0.8)

# ── Panel (2,0): ρ_0 and r_half ───────────────────────────────────────────────

ax20 = *sax(fig35, gs35, 2, 0, log_x=False, log_y=False)
ax20_r = ax20.twinx()
ax20.plot(time_arr, np.log10(np.where(rho0_arr > 0, rho0_arr, np.nan)),
color=”#e8673a”, lw=1.5, label=r”$\log*{10}\rho_0$”)
...
late_mi = n_maps - 1
ax31_.imshow(*log10_smooth(maps_3d[late_mi]).T, origin=“lower”, aspect=“equal”,
extent=MAP_EXTENT, cmap=“inferno”, vmin=vmin_2d, vmax=vmax_2d)
ax31*.set_title(f”Late  Σ(x,y)  t={time_maps[late_mi]:.2f} Gyr”
