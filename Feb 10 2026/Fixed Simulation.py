# =============================================================================
# MW–M31 Merger Simulation (Unit-Consistent, Stabilized)
# =============================================================================
...
# =============================================================================
# CONSTANTS (Fully kpc–Gyr System)
# =============================================================================
KM_S_TO_KPC_GYR = 1.0227121650537077

# Convert G from kpc (km/s)^2 / Msun  →  kpc^3 / (Msun Gyr^2)
...
class InitialConditionFactory:
    ...
    def hernquist_halo(comp: GalaxyComponent):
        ...
        # Stable isotropic dispersion approximation
        sigma = np.sqrt(G*M/(12*a))
        v = sigma * np.random.randn(n,3)
    ...
    def exponential_disk(comp: GalaxyComponent):
        ...
        M_enc = M * (1 - np.exp(-R/Rd)*(1 + R/Rd))
        vphi = np.sqrt(G * M_enc / (R + 1e-3))
...
def total_momentum(state):
    return np.sum(state[:,3:6] * state[:,6,None], axis=0)

def total_energy(state, eps):
    pos = state[:,0:3]
    vel = state[:,3:6]
    m = state[:,6]
...
class BarnesHutTree:
    ...
    def _build_node(self, center, half, idx):
        ...
        for k in range(8):
            mask = keys == k
            if np.any(mask):
                offset = ((np.array([(k>>2)&1,(k>>1)&1,k&1]) - 0.5)
                          * half)
    ...
    def _accel(self, i, node):
        ...
        # Correct opening criterion (use full size 2*half)
...
# =============================================================================
# LEAPFROG INTEGRATOR (Clean Units)
# =============================================================================
...
class MergerSimulation:
    ...
    def initialize(self):
        ...
        # Set relative orbit (convert km/s to kpc/Gyr)
        v_rel = np.array([-109.0,20.0,0]) * KM_S_TO_KPC_GYR
        r_sep = np.array([780.0,0,0])

        mw_mask = state[:,7] < 2
        m31_mask = state[:,7] >= 2

        mw_p,_ = center_of_mass(state, mw_mask)
        m31_p,_ = center_of_mass(state, m31_mask)

        state[m31_mask,0:3] += r_sep - (m31_p - mw_p)
        state[m31_mask,3:6] += v_rel
    ...
    def run(self):
        ...
        for step in range(self.config.n_steps):
            ...
            if step % 200 == 0:
                print("Momentum:", total_momentum(state))
                print("Energy:", total_energy(state, self.config.softening))
...
if __name__ == "__main__":
    ...
    for s in traj:
        ...
        sep.append(np.linalg.norm(m31_p - mw_p))

    t = np.arange(len(sep))*config.dt*config.snapshot_stride

    plt.plot(t, sep)
    plt.xlabel("Time [Gyr]")
    plt.ylabel("MW–M31 separation [kpc]")
    plt.title("MW–M31 Merger (Stabilized Toy Model)")
    plt.show()
