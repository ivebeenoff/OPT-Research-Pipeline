...
# The following are natural next steps that build on this pipeline:

# 

# 1. CIRCULAR VELOCITY  v_c(r) = sqrt(G M_enc(r) / r)

# Add a dedicated profile column and plot alongside v_esc to compare the

# depth of the potential at different epochs.

# 

# 2. JEANS EQUATION  σ_r  VERIFICATION

# Solve the Jeans equation using the measured density and σ_r profiles to

# check for equilibrium.  Deviations flag snapshots that are kinematically

# out of equilibrium (e.g., right after pericentre).

# 

# 3. TIDAL STREAM IDENTIFICATION

# Particles with β near +1 and large specific j at large r are likely

# tidal-stream members.  Cluster them in (r, β, j) space and track their

# debris over time.

# 

# 4. SEPARATE MW AND M31 PROFILES

# Tag particles by their galaxy of origin and compute separate σ_r(r, t)

# for each to track mixing and heating individually.

# 

# 5. ANIMATION

# Loop over the saved time-series arrays and call matplotlib.animation

# to produce an mp4 of the radial profile evolution — no re-loading required.

# 

# 6. HALO CONCENTRATION

# Fit an NFW profile to M_enclosed_bin(r) at each snapshot to track how

# the concentration parameter c evolves through the merger.
